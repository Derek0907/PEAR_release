...
class Fusion_hot_pose(nn.Module):
    '''
    verb_feature: [bs, 32, 768]
    hot_feature: [bs, 1, 224, 224]
    '''
    def __init__(self, input_dim = 768, condition_dim = 768, depth = 2):
        super(Fusion_hot_pose, self).__init__()
        self.input_dim = input_dim
        self.condition_dim = condition_dim
        self.depth = depth
        self.cross_attn = CrossAttention(input_dim, condition_dim)
        self.pos_embed_input = nn.Parameter(torch.zeros(1, 32, input_dim))
        self.pos_embed_condition = nn.Parameter(torch.zeros(1, 196, condition_dim))
        self.norm = nn.LayerNorm(input_dim)
        self.patch_embed = PatchEmbed(height=224, width=224, patch_size=16, in_c=1, embed_dim=input_dim)
        self.mlp = Mlp(in_features=input_dim, hidden_features=3*input_dim, act_layer=nn.GELU, drop=0.)
    ...
    def forward(self, x_input, x_condition):
        '''
        x_input: [B, 32, 768]
        x_condition: [B, 1, 224, 224]
        '''
        B = x_input.shape[0]
        x_condition = self.patch_embed(x_condition) #[B, 196, 768]
        ...
        for i in range(self.depth):
            ...
            x_input= x_input + self.mlp(self.norm(x_input)) #[B, 32, 768]
        return x_input
